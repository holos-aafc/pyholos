...
class _HolosTable:
    def __init__(
            self,
            name: str,
            data: list[dict[str, Any]] | DataFrame,
            path: Path
    ):
        self.name = name
    ...
    @staticmethod
    def _ensure_dataframe(data: list[dict[str, Any]] | DataFrame) -> DataFrame:
        """Ensures that the data is in DataFrame format."""
    ...
    def write_data_to_csv(
        ...
        with self.path.open(mode='w', newline='') as f:
            ...
            self.data.to_csv(f, sep=',', decimal='.', index=False)
...
def set_table_16():
    ...
    table_16.write_data_to_csv(
        comments=[
            ...
            (
                'source: '
                'https://github.com/holos-aafc/Holos/blob/396f1ab9bc7247e6d78766f9445c14d2eb7c0d9d/H.Core/Providers/Animals/Table_16_Livestock_Coefficients_BeefAndDairy_Cattle_Provider.cs#L13'  # noqa: E501
            ),
            '',
            ('The value of "DefaultFinalWeight" was modified from 260 kg to 90 kg,'
             ' according to the default values set in the GUI.')
        ])
...
def set_table_30():
    ...
    Holos Source Code Footnotes:
        Footnote 1:
            C, N and P composition values for straw (barley) and wood-chip bedding are averages of data recorded
            in 1998 and 1999 in Larney et al. (2008); straw bedding application rates for beef cattle are from
            Chai et al. (2014), originally based on Larney et al. (2008) and Gilhespy et al. (2009)
        Footnote 2:
            Wood-chip bedding is a mixture of sawdust and bark peelings derived from 80% lodgepole pine
            (Pinus contorta var. latifolia  Engelm.) and 20% white spruce [Picea glauca (Moench)Voss].
        Footnote 3:
            “Drylot” refers to milking parlours, yards and exercise lots.
        Footnote 4:
            Values for bedding amounts are based on minimum values for recommended bedding per 450 kg animal weight
            from Lorimor et al. (2004, Table 13).
            We assumed an average dairy cow body weight of 687 kg (Lactanet, 2020)
                - sand for freestall barn: 15.9 kg/450 kg * 687 kg = 24.3 kg head-1 day-1;
                - shavings and sawdust for tie-stall barn: 1.4 kg/450 kg * 687 kg = 1.9 kg head-1 day-1
            due to a lack of available data, bedding application rates for sand for freestall barns and shavings and
            sawdust for freestall barns were also applied to other dairy housing types.
            For swine, calculations assumed an average weight for sows and boars of 198 kg (ECCC, 2022)
                - chopped straw: 1.8 kg/450 kg * 198 kg = 0.79 kg head-1 day-1;
                - long straw: 1.6 kg/450 kg * 198 kg = 0.7 kg head-1 day-1
            For poultry we assumed an average weight of (ECCC, 2022):
                - 0.9 kg for broilers
                - 1.8 kg for layers
                - 6.8 kg for turkeys.
                For sawdust bedding this gave application rates of:
                    - broilers: 0.7 kg/450 kg * 0.9 kg = 0.0014 kg head-1 day-1;
                    - layers: 0.7 kg/450 kg * 1.8 kg = 0.0028 kg head-1 day-1;
                    - turkeys: 0.7 kg/450 kg * 6.8 kg = 0.011 kg head-1 day-1.
        Footnote 5:
            Dairy manure soilds separated from manure liquid using a screw press and composted and dried; also sometimes
            referred to as recycled manure solids, dried manure solids, undigested feedstuffs or more commonly
            compost bedding (OMAFRA, 2015). Values from Misselbrook and Powell (2005, Table 1).
        Footnote 6:
            Values for long and chopped straw follow Chai et al. (2016) and are originally from Rotz et al. (2013).
        Footnote 7:
            Due to a lack of data, total C, total N, total P and C:N ratio values for wood-chip bedding from
            Larney et al. (2008) are used for wood shavings and sawdust bedding for dairy cattle, sheep, swine
            and poultry.
        Footnote 8:
            Bedding application rates for sheep were obtained from the Canadian Sheep Foundation (2021), and applied to
            all bedding options. The application rate of 0.57 was calculated as the midpoint of the recommended
            0.45-0.68 kg head-1 day-1 range provided.
        Footnote 9:
            Following Chai et al. (2016), a total N content value of 0.0057 (derived from Larney et al., 2008) was used
            for chopped straw bedding for dairy cattle; due to a lack of data, the total C, total N, total P and
            C:N ratio values for straw bedding from Larney et al. (2008) were applied to long- and chopped-straw bedding
            for dairy cows, swine and poultry.
        Footnote 10:
            Bedding options for sheep were identified from the Canadian Sheep Foundation (2021). Nutrient concentration
            values for straw and wood shavings for beef cattle were used for sheep in a drylot/corral and barn.
        Footnote 11:
            For Other livestock, straw was assumed to be the main bedding type and total C, total N, total P and
            C:N ratio values for straw bedding from Larney et al. (2008) were applied. For llamas and alpacas and goats,
            the bedding application rate for sheep was used; for deer and elk, horses, mules and bison, the bedding
            application rate for beef cattle (feedlot) was used as no data for these animals groups was available.
        Footnote  12:
            Values for moisture content of bedding from Ferraz et al. (2020). The moisture content for straw is the mean
            of values for barley straw (9.8%) and wheat straw (9.33%); the moisture content for sawdust is the value for
            dried sawdust.
    """
    ...
    table_30.write_data_to_csv(
        comments=[
            'Table 30. Default bedding application rates and composition of bedding materials for all livestock groups.',  # noqa: E501
            'source: https://github.com/holos-aafc/Holos/blob/396f1ab9bc7247e6d78766f9445c14d2eb7c0d9d/H.Core/Providers/Animals/Table_30_Default_Bedding_Material_Composition_Provider.cs#L15'  # noqa: E501
        ])
    pass
...
def set_tillage_factor_table():
    ...
    table_tillage.write_data_to_csv(
        comments=[
            'Holos source code: https://github.com/holos-aafc/Holos/blob/e644d8e52446faefe3d7503565a723563bba61fe/H.Core/Calculators/Tillage/TillageFactorCalculator.cs#L27',  # noqa: E501
        ])
